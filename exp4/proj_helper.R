...
foci_proj <- function(Y, X, XPCA, real=FALSE, num_hidden=NULL, proj=FALSE, num_features = NULL, stop = TRUE, numCores = parallel::detectCores()){
  ...
  # Add invertible projections
  if (proj == FALSE){
    Z_est <- Z_est %*% diag(num_hidden)
  } else {
    # Construct P
    set.seed(2021)
    U <- randortho(num_hidden, type = "orthonormal")
    delta <- runif(num_hidden, 0, 1)
    nrs <- 1 + delta
    d <- diag(x = nrs)
    P <- U %*% d %*% t(U)
    # New Z
