...
foci_main <- function(Y, X, num_features = NULL, stop = TRUE, numCores = parallel::detectCores()){
  ...
  while (count < num_features) {
    ...
    # return the single signal estimated Q function but not combined
    estimateQFixedYandIndepX <- function(id){
      return(.estimateQ(Y, X[, id]))
    }
    trueQ[count + 1] <- estimateQFixedYandIndepX(index_left[index_max])
  ...
  stepT = Q / .estimateS(Y)
  stepCODEC <- trueQ / .estimateS(Y)
  result = list(selectedVar = selectedVar, stepT = stepT[1:count], 
                stepCODEC = stepCODEC[1:count])
  class(result) = "foci"
  return(result)
}

foci_new <- function(Y, X, ZVAE, real=FALSE, num_features = NULL, stop = TRUE, numCores = parallel::detectCores()){
  ...
  stepCODEC <- trueQ / .estimateConditionalS(Y, Z_est)
  result = list(selectedVar = selectedVar, stepT = stepT[1:count], 
                stepCODEC = stepCODEC[1:count])
  result
}
