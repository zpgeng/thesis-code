...
# Defining new function to put estimated hidden always in the conditioning set.

foci_new <- function(Y, X, num_hidden, real=FALSE, num_features = NULL, stop = TRUE, numCores = parallel::detectCores()){
  ...
  if (real==FALSE){
    ...
    Z_est <- X.PCA$x[1:n, 1:num_hidden]
  }
  else{
    Z_est <- real
  }
